...
st.markdown("""
    <style>
        ...
        /* Asegurar que las columnas de resultados no salten en móviles pequeños */
...
# 2. Carga de Datos con Filtro Comercial
...
def cargar_datos():
    ...
    try:
        ...
        datos_brutos = r.json()["ListaEESSPrecio"]
        
        # FILTRO ESTRICTO: Solo guardamos gasolineras abiertas al público general
        datos_limpios = [g for g in datos_brutos if g.get("Tipo Venta") == "P"]
        
        # Guardamos el backup ya limpio
        pd.DataFrame(datos_limpios).to_csv(archivo_backup, index=False)
        return datos_limpios, datetime.datetime.now(tz_madrid)
    except Exception:
        ...
        if not res.empty:
            for _, g in res.head(20).iterrows():
                with st.container(border=True):
                    ...
                    with col_btn:
                        lat_rut = str(g['lat_num']).replace(",", ".")
                        lon_rut = str(g['lon_num']).replace(",", ".")
                        url_map = f"https://www.google.com/maps?q={lat_rut},{lon_rut}"
