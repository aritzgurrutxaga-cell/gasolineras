...
st.markdown("""
    <style>
        ...
        /* DISEÑO DE LA CAJA DE TEXTO (Altura garantizada para que el municipio respire) */
        ...
        /* --- BOTÓN INICIAL (AISLADO EN SU CAJA PARA NO CONTAGIAR A LOS DEMÁS) --- */
        .caja-boton-gigante div[data-testid="stButton"] button {
        ...
        .caja-boton-gigante div[data-testid="stButton"] button:active {
            transform: translateY(2px) !important;
            box-shadow: 0 2px 3px rgba(0,0,0,0.2) !important;
        }
        
        .caja-boton-gigante div[data-testid="stButton"] button p {
            font-size: 1.5rem !important;
            font-weight: bold !important;
            margin: 0 !important;
        }
        
        .caja-boton-gigante div[data-testid="stButton"] button::after {
...
if not (estado_permiso == "granted" or st.session_state.municipio_guardado) and not st.session_state.solicitar_gps:
    ...
    # EL BOTÓN GIGANTE ESTÁ AISLADO AQUÍ. Imposible que contagie a otros botones.
    st.markdown('<div class="caja-boton-gigante">', unsafe_allow_html=True)
...
if not lat_gps and not st.session_state.municipio_guardado:
    ...
    if st.button("✅ Confirmar selección", use_container_width=True):
        if municipio_sel:
            ...
            # Sincronizamos las variables de memoria
            st.session_state.draft_muni = municipio_sel
            st.session_state.last_muni_ref = municipio_sel
...
if 'draft_muni' not in st.session_state or st.session_state.get('last_muni_ref') != muni_ref:
    st.session_state.draft_muni = muni_ref
    st.session_state.last_muni_ref = muni_ref
...
# --- EL EXPANDER DOMINADO ---
with st.expander("⚙️ Ajustes de búsqueda"):
    # Usamos 'key' en lugar de 'index' para que no colisionen al tocarlos
    ...
    if st.button("🔍 Buscar", use_container_width=True):
        # Volcamos los borradores a la memoria real
